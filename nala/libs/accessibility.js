...
import { AccessibilityError } from './nalaerrors.js';
 ...
 */
async function runAccessibilityTest(page, testScope = 'body', includeTags = ['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'], maxViolations = 0) {
  let testName;
  try {
    // Retrive the test title (name)
    testName = test.info().title.includes(',') ? test.info().title.split(',')[0] : test.info().title;
    ...
    const enhancedAxeResults = {
      testName,
      testScope: testScope === 'body' ? 'Entire Page' : `Specific Section: ${testScope}`,
      ...axe,
    };
    ...
    if (violationCount > maxViolations) {
      ...
      console.info(`[Accessibility Test]: Found ${violationCount} violation(s) for ${testName}, but under the threshold of ${maxViolations}.`);
    } else {
      console.info(`[Accessibility Test]: No accessibility violations found for ${testName}.`);
    }
  } catch (err) {
    throw new AccessibilityError(`[Accessibility Test failed for ${testName}].\n ${err.message}`);
  }
}
